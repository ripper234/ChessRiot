...
export interface MagicSequenceContext {
  initialPiece: MagicPiece;
  maxMoves: number;
}
...
export function magicContinuationStep(
  ...
  sequence?: MagicSequenceContext,
): MagicContinuationStep | null {
  if (chessAfterMove.isCheck()) return null;
  const piece = chessAfterMove.get(pieceSquare);
  const initialPiece = sequence?.initialPiece ?? piece?.type;
  const maxMoves = sequence?.maxMoves
    ?? (initialPiece ? magicMoveLimit(rules, initialPiece) : 1);
  if (
    !piece
    || !initialPiece
  ...
  return moves.length > 0
    ? { chess, moves, piece: piece.type, initialPiece, maxMoves }
    : null;
}
...
function applyActionToPosition(
  ...
  const sequence: MagicSequenceContext = {
...
export function applyCandidate(
  ...
  const applied = applyActionToPosition(chess, candidate, rules);
  chess = applied.chess;
