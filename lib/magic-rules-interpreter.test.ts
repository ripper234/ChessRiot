...
import {
  ...
  MAGIC_COMPILER_MAX_OUTPUT_TOKENS,
...
describe("runtime Magic interpreter", () => {
  ...
  it("rejects the entire prompt when any clause is unsupported", async () => {
    ...
    await expect(interpretMagicPrompt(
      ...
      ok: false,
      code: "unsupported",
      message: "That rule is not supported yet. Try extra moves or disable promotion, castling, or en passant.",
    });
  });
  ...
  it.each([
  ...
  ] as const)("classifies provider HTTP %s", async (status, code) => {
    ...
    await expect(interpretMagicPrompt(
      "Knights move twice",
      { apiKey: "test-key", fetch: fetchMock as typeof fetch },
    )).resolves.toMatchObject({ ok: false, code, providerStatus: status });
  });

  it("classifies transport failures without calling them rule errors", async () => {
    const fetchMock = vi.fn(async () => {
