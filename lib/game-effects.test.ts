...
function snapshot(moves: PublicMove[]): GameSnapshot {
  const fen = moves.at(-1)?.fenAfter ?? new Chess().fen();
  return {
    id: "game",
    mode: "multiplayer",
    aiDifficulty: null,
...
describe("boardEffects", () => {
  ...
  it("attacks as a pawn before revealing a capture promotion", () => {
    ...
    expect(new Chess(effect.afterFen).get("h8")).toMatchObject({
      color: "w",
      type: "q",
    });
  });
  ...
  it("emits both legs of an atomic Magic capture", () => {
    ...
    expect(effects.map((effect) => ({
      ...
      {
        id: "1:first:a1-a3",
        attacker: { color: "w", type: "r" },
        victim: { color: "b", type: "p", square: "a3" },
      },
      {
        id: "1:second:a3-a5",
        attacker: { color: "w", type: "r" },
        victim: { color: "b", type: "p", square: "a5" },
      },
    ]);
  });

  it("skips animation when a stored move cannot be reconstructed safely", () => {
