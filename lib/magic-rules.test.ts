...
describe("Magic Rules documents", () => {
  ...
  it("validates canonical v3 move counts and produces stable labels", () => {
    ...
    expect(magicRuleLabel(TRIPLE_KNIGHT.rules[0], "he"))
      .toBe("אפשר להזיז פרשים עד 3 פעמים בתור; שח מסיים את התור");
  });

  it("localizes every protected rule label without changing server defaults", () => {
    expect(magicRuleLabel({ kind: "no_promotion" })).toBe("Pawns cannot move onto the final rank");
    expect(magicRuleLabel({ kind: "no_promotion" }, "he"))
      .toBe("חיילים אינם יכולים להגיע לשורה האחרונה");
    expect(magicRuleLabel({ kind: "no_castling" }, "he")).toBe("אין הצרחה");
    expect(magicRuleLabel({ kind: "no_en_passant" }, "he")).toBe("אין הכאה דרך הילוכו");
  });
