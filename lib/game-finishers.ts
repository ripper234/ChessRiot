...
export function classifyGameFinisher(
  ...
  try {
    ...
    const final = lastMove.continuation?.at(-1) ?? lastMove.second;
    const attackingTo = (final?.to ?? lastMove.to) as Square;
    ...
    return {
      ...
      attackingFrom: (final?.from ?? lastMove.from) as Square,
      attackingTo,
