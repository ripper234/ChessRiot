...
export function classifyGameFinisher(
  ...
  try {
    ...
    const finalLeg = lastMove.continuation?.at(-1) ?? lastMove.second;
    const attackingTo = (finalLeg?.to ?? lastMove.to) as Square;
    ...
    return {
      ...
      attackingFrom: (finalLeg?.from ?? lastMove.from) as Square,
      attackingTo,
