...
export async function getChatGPTUser(): Promise<ChatGPTUser | null> {
  ...
  return {
    displayName: fullName ?? email.split("@")[0] ?? "Player",
    email,
    fullName,
  };
}
...
export async function requirePlayableAccount(
  returnTo: string,
): Promise<PlayerAccount> {
  await requireChatGPTUser(returnTo);
  const account = await verifiedAccountFromHeaders(await headers());
  if (account) return account;
  redirect(`/verify?return_to=${encodeURIComponent(safeRelativeReturnPath(returnTo))}`);
}
